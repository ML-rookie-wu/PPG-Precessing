...
def read_package_from_port(dir, spo2):
...
def collect_data_spo2(read_func):

    # read_time = 10
    # data_dir = os.path.join(Path(__file__).parent.parent, 'data')
    dir = r'E:\my_projects\PPG\data\spo2_compare_new\5disturb_4pulse'
    spo2 = 80
    for i in range(5):
        path = read_func(dir, spo2)

def collect_data_pulse(read_func):

    # read_time = 10
    # data_dir = os.path.join(Path(__file__).parent.parent, 'data')
    dir = r'E:\my_projects\PPG\data\pulse_compare\5disturb'
    pulse = '30'
    for i in range(2):
...
def collect_real_data(save_path=None):
    if save_path is not None:
        path = save_path
    else:
        dir = r'D:\my_projects_V1\my_projects\PPG_V1\data\real_data\wu\resp'
...
def collect_resp_data(save_path=None):
    if save_path is not None:
        path = save_path
        if os.path.isdir(save_path):
            path = os.path.join(save_path, time.strftime("%Y%m%d%H%M%S", time.localtime()) + '.txt')
    else:
